...
Field definitions for invoices — PARENT (document-level):
  ...
  "Payment Terms": payment terms text when present (e.g. "Net 30", "net within 14 days", "Due on receipt").
                   Capture from labels like Payment Terms, Terms of Payment, Terms, Payment Condition.
...
def _parse_extraction_response(
    ...
    # Gemini occasionally returns a top-level JSON array ([{...}]) even though the
    # prompt asks for a single object. Unwrap the first dict instead of crashing.
    if isinstance(data, list):
        data = next((x for x in data if isinstance(x, dict)), None)
    if not isinstance(data, dict):
        logger.error(
            "  [extract] %s: unexpected JSON shape (%s); expected an object.",
            file_name,
            type(data).__name__,
        )
        return [{"Folder": folder_name, "Filename": file_name, **{f: "" for f in FIELDS}}]
    ...
    if is_invoice:
        ...
        if not (base.get("Payment Terms") or "").strip():
            base["Payment Terms"] = _first_non_empty(
                parent,
                ["payment_terms", "Terms of Payment", "Payment terms", "payment terms", "terms_of_payment", "Terms"],
            )
