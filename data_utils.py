...
def _build_contract_id_groups(raw_ids: List[str]) -> Dict[str, str]:
    ...
    IDs group when they are suffix variants of the same contract, e.g.:
    - '4533232908' + '4533232908-1' → '4533232908'
    - '1111-2' + '1111-3' (no bare '1111' row) → '1111'
    - '11-005' + '11-005-1' → '11-005' when bare '11-005' exists in the sheet
    """
    ...
    # All normalized IDs that share the same strip base (e.g. 1111-2 and 1111-3 → base 1111).
    base_to_members: Dict[str, set] = {}
    for rid in raw_ids:
        norm = normalized[rid]
        strip_base, _ = stripped[rid]
        base_to_members.setdefault(strip_base, set()).add(norm)

    mapping: Dict[str, str] = {}
    for rid in raw_ids:
        norm = normalized[rid]
        base, _suffix_num = stripped[rid]
        members = base_to_members.get(base, {norm})

        # Multiple suffix variants of the same strip base (1111-2 + 1111-3).
        suffix_family = len(members) > 1 and any(
            _strip_version_suffix(n)[1] > 0 for n in members
        )
        # Classic case: bare base exists and a -1 / -2 variant exists.
        bare_base_variant = base != norm and base in all_norm

        if suffix_family or bare_base_variant:
            mapping[rid] = base if base in members else base
        else:
            mapping[rid] = norm
    return mapping


def patch_sharepoint_urls_in_dataframe(
    df: Optional[pd.DataFrame],
    url_column: str,
    updates: List[Dict[str, str]],
) -> Optional[pd.DataFrame]:
    """
    Update SharePoint URL cells when a file moved but content is unchanged (same item id / eTag).
    Matches rows by Filename + previous URL so extracted fields are preserved.
    """
    if df is None or df.empty or not updates:
        return df
    if url_column not in df.columns or "Filename" not in df.columns:
        return df
    out = df.copy()
    for u in updates:
        fname = (u.get("filename") or "").strip()
        old_url = (u.get("old_url") or "").strip()
        new_url = (u.get("new_url") or "").strip()
        if not fname or not old_url or not new_url:
            continue
        mask = (
            out["Filename"].astype(str).str.strip() == fname
        ) & (out[url_column].astype(str).str.strip() == old_url)
        if mask.any():
            out.loc[mask, url_column] = new_url
    return out


def _url_column(df: pd.DataFrame) -> Optional[str]:
    for col in ("SharePoint URL", "SharePoint Link"):
        if col in df.columns:
            return col
    return None


def _version_sort_key_from_indices(df: pd.DataFrame, indices: list) -> Tuple:
    """Sort key for a set of rows from one file/copy (latest Start/End/suffix wins)."""
...
def _collapse_duplicate_folder_copies(
    df: pd.DataFrame,
    indices: list,
    url_col: Optional[str],
) -> Tuple[list, list]:
    """
    Same Filename + Contract ID in multiple folders (different SharePoint URLs).
    Keep rows from the copy with the latest Start/End/suffix; drop the rest.
    """
    if not url_col or len(indices) < 2:
        return indices, []
    by_url: Dict[str, list] = {}
    for i in indices:
        u = str(df.loc[i, url_col] or "").strip() or "__no_url__"
        by_url.setdefault(u, []).append(i)
    if len(by_url) <= 1:
        return indices, []
    winner_url = max(by_url.keys(), key=lambda u: _version_sort_key_from_indices(df, by_url[u]))
    kept = by_url[winner_url]
    dropped = [i for u, ids in by_url.items() if u != winner_url for i in ids]
    return kept, dropped
...
def dedupe_by_contract_id(
    ...
    This handles:
      ...
      c) Same filename copied to another folder (different SharePoint URL) — one copy kept
    ...
    for group_base, file_versions in buckets.items():
        # Same filename in multiple folders → keep one copy (ignore URL for version pick).
        collapsed: Dict[str, list] = {}
        for fname, indices in file_versions.items():
            kept, folder_dropped = _collapse_duplicate_folder_copies(df, indices, url_col)
            collapsed[fname] = kept
            if folder_dropped:
                drop_indices.update(folder_dropped)
                kept_key = _version_sort_key_from_indices(df, kept)
                kept_start = kept_key[0].isoformat() if kept_key[0] != pd.Timestamp.min else ""
                dedup_log.append({
                    "dropped_file": fname,
                    "dropped_contract_ids": sorted(
                        set(str(df.loc[i, "Contract ID"]) for i in folder_dropped)
                    ),
                    "kept_file": fname,
                    "kept_contract_id": sorted(set(str(df.loc[i, "Contract ID"]) for i in kept)),
                    "group_base": group_base,
                    "reason": (
                        "Contract ID duplicate: same filename in multiple folders; "
                        f"kept copy with latest dates{f' (Start {kept_start})' if kept_start else ''}"
                    ),
                    "dedup_type": "contract_id",
                })
        file_versions = {fn: idx for fn, idx in collapsed.items() if idx}
        if len(file_versions) <= 1:
            continue

        def _version_sort_key(fname: str) -> Tuple:
            return _version_sort_key_from_indices(df, file_versions[fname])
