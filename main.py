...
from data_utils import (
    ...
    patch_sharepoint_urls_in_dataframe,
...
def _run_extraction(
    ...
    path_url_updates: list = []
    processed_this_batch = 0
    SAVE_EXCEL_EVERY_N = 10  # periodic local saves during long runs (GCS upload at end/stop only)

    def _skip_or_update_path_only(item: dict) -> bool:
        """True when item should not be re-extracted (unchanged file or path-only move)."""
        if force_reprocess:
            return False
        uid = item.get("id")
        if not uid or uid not in processed_ids:
            return False
        status = classify_item(item, processed_ids, processed_meta)
        if status != "up_to_date":
            return False
        meta = processed_meta.get(uid) or {}
        old_url = (meta.get("webUrl") or "").strip()
        new_url = (item.get("webUrl") or "").strip()
        if new_url and old_url and new_url != old_url:
            path_url_updates.append({
                "filename": (item.get("name") or "").strip(),
                "old_url": old_url,
                "new_url": new_url,
                "doc_type": (meta.get("doc_type") or "sow").strip(),
            })
            mark_processed(
                uid,
                name=item.get("name"),
            ...
            processed_meta[uid] = {**meta, "webUrl": new_url}
            logger.info(
                "SharePoint link updated without re-extraction: %s",
                item.get("name"),
            )
        return True
    ...
    def _apply_path_url_updates() -> None:
        nonlocal existing_sow_df, existing_inv_df
        if not path_url_updates:
            return
        sow_updates = [u for u in path_url_updates if u.get("doc_type") != "invoice"]
        inv_updates = [u for u in path_url_updates if u.get("doc_type") == "invoice"]
        existing_sow_df = patch_sharepoint_urls_in_dataframe(
            existing_sow_df, _URL_KEY, sow_updates
        )
        existing_inv_df = patch_sharepoint_urls_in_dataframe(
            existing_inv_df, _URL_KEY, inv_updates
        )
        logger.info(
            "Patched SharePoint URL(s) in Excel for %s moved file(s) (no re-extraction).",
            len(path_url_updates),
        )

    def _save_and_return(upload: bool = True):
        _apply_path_url_updates()
    ...
    while True:
        ...
        item, drive_id = x
        idx += 1
        if _skip_or_update_path_only(item):
            continue
