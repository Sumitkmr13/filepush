...
def verify_sharepoint_path_reachable(
    ...
    try:
        ...
        # Try direct path first (handles special chars better than segment walk).
        direct_url = f"{GRAPH_BASE}/drives/{drive_id}/root:/{quote(folder_path, safe='')}:/children"
        r = requests.get(direct_url, headers={"Authorization": f"Bearer {token}"}, timeout=GRAPH_REQUEST_TIMEOUT)
        if r.status_code == 200:
            data = r.json()
            value = data.get("value", [])
            summary = _format_first_page_summary(value)
            return True, f"path reachable (direct), first page: {summary}"
...
def list_pdf_items_streaming(
    ...
    # Try Graph Search API first (fast for large drives), but fall back to recursive
