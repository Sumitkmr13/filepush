...
GCP: On Cloud Run, set GCP_PROJECT and use the runtime service account (ADC); optional
GOOGLE_APPLICATION_CREDENTIALS for local/dev JSON key. Project is also read from that JSON if GCP_PROJECT is unset.
...
FIELDS = [
    ...
    "Start Date",
    "End Date",
    "Description",  # invoice: note when PO date used as Start Date because no dates extracted
]

# Columns written to contract_metrics.xlsx (excludes Document Type; Filename is always first)
SOW_FIELDS = [
    "Original Language",
    "Contract ID",
    "Vendor",
...
# Background extraction auto-stop after N minutes (saves Excel/state like manual Stop). 0 = no limit.
EXTRACTION_MAX_RUN_MINUTES = int(os.environ.get("EXTRACTION_MAX_RUN_MINUTES", "240") or "0")
